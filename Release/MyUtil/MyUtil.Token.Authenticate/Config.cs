        ...
        internal static bool? CHECK_ADDRESS
        {
            get
            {
                if (!checkAddress.HasValue)
                {
                    try
                    {
                        checkAddress = int.Parse(ConfigurationManager.AppSettings["MyUtil.Token.Authenticate.CheckAddress"]) == 1;
                    }
                    catch (Exception ex)
                    {
                        LogSystem.Error(ex);
        ...
        internal static double TOKEN_TIMEOUT
        {
            get
            {
                if (tokenTimeout <= 0)
                {
                    try
                    {
                        tokenTimeout = double.Parse(ConfigurationManager.AppSettings["MyUtil.Token.Authenticate.TokenTimeout"]);
                    }
                    catch (Exception ex)
                    {
                        LogSystem.Error(ex);

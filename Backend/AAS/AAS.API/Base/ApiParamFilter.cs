...
namespace AAS.API.Base
{
    public class ApiParamFilter : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuting(HttpActionContext actionContext)
        {
            try
            {
                LogSystem.Debug(string.Format("User: {0}, Controller: {1}, Action: {2}", BackendTokenManager.GetLoginname(), actionContext.ControllerContext.ControllerDescriptor.ControllerName, actionContext.ActionDescriptor.ActionName));

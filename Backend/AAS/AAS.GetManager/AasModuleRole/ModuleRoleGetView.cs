...
namespace AAS.GetManager.AasModuleRole
{
    ...
    {
        internal List<ViewModuleRole> GetView(AasModuleRoleViewFilter filter)
        {
            try
            {
                StringBuilder sb = new StringBuilder().Append("SELECT * FROM \"ViewModuleRole\"");
                ...
                return DAOWorker.SqlDAO.GetSql<ViewModuleRole>(sqlQuery);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
        ...
        private string GetFilterQuery(AasModuleRoleViewFilter filter)
        {

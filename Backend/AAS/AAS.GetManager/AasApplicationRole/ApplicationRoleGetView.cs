...
namespace AAS.GetManager.AasApplicationRole
{
    ...
    {
        internal List<ViewApplicationRole> GetView(AasApplicationRoleViewFilter filter)
        {
            try
            {
                StringBuilder sb = new StringBuilder().Append("SELECT * FROM \"ViewApplicationRole\"");
                ...
                return DAOWorker.SqlDAO.GetSql<ViewApplicationRole>(sqlQuery);
            }
            catch (Exception ex)
            {
                LogSystem.Error(ex);
        ...
        private string GetFilterQuery(AasApplicationRoleViewFilter filter)
        {

...
{
    partial class AasModuleCheck : BusinessBase
    {
        internal AasModuleCheck()
            : base()
        {

        }

        internal AasModuleCheck(CommonParam paramCheck)
            : base(paramCheck)
        {
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool VerifyId(long id, ref Module data)
        {
            bool valid = true;
            try
            {
                data = new ModuleManagerGet().GetById(id);
                if (data == null)
                {
                    BugUtil.SetBugCode(param, LibraryBug.Bug.Enum.Common__KXDDDuLieuCanXuLy);
                    Logging("Id invalid." + LogUtil.TraceData("id", id), LogType.Error);
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(Module data)
        {
            bool valid = true;
            try
            {
        ...
        internal bool IsUnLock(long id)
        {
            ...
            try
            {
                if (!DAOWorker.AasModuleDAO.IsUnLock(id))
                {
                    valid = false;
                    MessageUtil.SetMessage(param, LibraryMessage.Message.Enum.Common__DuLieuDangBiKhoa);
                }
        ...
        /// <param name="data"></param>
        /// <returns></returns>
        internal bool IsUnLock(List<Module> listData)
        {
            bool valid = true;
            try
            {
